...
export class ShipmentComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['trackingCode', 'details'];

  subscription: Subscription | null = null;
  shipments = signal<IShipment[]>([]);
  isLoading = false;
  ...
  trackId = (item: IShipment): number => this.shipmentService.getShipmentIdentifier(item);
  ...
  search(query: string): void {
    ...
    if (query && predicate && ShipmentComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && ShipmentComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.shipments.set(dataFromBody);
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.shipmentService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
