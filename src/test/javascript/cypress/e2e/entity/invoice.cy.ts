...
describe('Invoice e2e test', () => {
  ...
  const invoiceSample = {
    date: '2025-07-31T19:30:33.435Z',
    status: 'PAID',
    paymentMethod: 'CREDIT_CARD',
    paymentDate: '2025-08-01T00:16:58.074Z',
    paymentAmount: 27895.62,
  };
  ...
  describe('new Invoice page', () => {
    ...
    it('should create an instance of Invoice', () => {
      cy.get(`[data-cy="date"]`).type('2025-07-31T08:15');
      cy.get(`[data-cy="date"]`).blur();
      cy.get(`[data-cy="date"]`).should('have.value', '2025-07-31T08:15');

      cy.get(`[data-cy="details"]`).type('ack coaxingly');
      cy.get(`[data-cy="details"]`).should('have.value', 'ack coaxingly');

      cy.get(`[data-cy="status"]`).select('CANCELLED');

      cy.get(`[data-cy="paymentMethod"]`).select('CREDIT_CARD');

      cy.get(`[data-cy="paymentDate"]`).type('2025-08-01T03:44');
      cy.get(`[data-cy="paymentDate"]`).blur();
      cy.get(`[data-cy="paymentDate"]`).should('have.value', '2025-08-01T03:44');

      cy.get(`[data-cy="paymentAmount"]`).type('29967.55');
      cy.get(`[data-cy="paymentAmount"]`).should('have.value', '29967.55');
