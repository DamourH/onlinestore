 ...
 */
public class ProductCategoryGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all productCategories").get("/api/product-categories").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new productCategory")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created productCategory").get("${new_productCategory_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created productCategory").delete("${new_productCategory_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ProductCategory entity").exec(scn);
