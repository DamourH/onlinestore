...
public class InvoiceGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all invoices").get("/api/invoices").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created invoice").get("${new_invoice_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created invoice").delete("${new_invoice_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
